...
def train(num_episode, agent, env, output, start_episode=0):
    ...
    finder = SensitiveLayerFinder(env.model, env.train_loader, device)
    update_period = 10
    rank_exps = finder.compute_rank_expectations(max_batches=1)
    sensitive_layers = finder.identify_sensitive_layers(rank_exps)
    env.sensitive_layers = sensitive_layers
    env.sensitive_ids = [i for i, name in enumerate(env.layer_names) if name in sensitive_layers]
    while episode < num_episode:  # counting based on episode
        ...
        if done:  # end of episode
            if episode % update_period == 0 and episode != start_episode:
                finder = SensitiveLayerFinder(env.model, env.train_loader, device)
                rank_exps = finder.compute_rank_expectations(max_batches=4)
                sensitive_layers = finder.identify_sensitive_layers(rank_exps)
                env.sensitive_layers = sensitive_layers
                env.sensitive_ids = [i for i, name in enumerate(env.layer_names) if name in sensitive_layers]
                sensitive_ids = []
                ...
                print("[Train] Sensitive layer indices:", sensitive_ids)
                text_writer.write("[Train] Sensitive layer indices: {}".format(sensitive_ids))
            ...
            text_writer.write('best policy: {}\n'.format([float(r) for r in env.best_strategy]))
...
if __name__ == "__main__":
    ...
    _, calib_loader, _ = get_split_dataset(args.dataset,
    ...
    if args.job == 'train':
        if args.resume != 'default':
            ...
            if args.rbound == 1.:
                base_folder_name = '{}_{}_r{}_ep{}_search'.format(args.model, 'sensitivity', args.preserve_ratio, args.train_episode)
            else:
                base_folder_name = '{}_{}_r{}_rbound{}_ep{}_search'.format(args.model, 'sensitivity', args.preserve_ratio, args.rbound, args.train_episode)
        ...
        run_info_path = os.path.join(args.output, 'run_info.txt')
        ...
        # Save run info for export
        import sys
        from datetime import datetime
        export_dir = os.path.dirname(args.export_path)
        export_filename_without_ext = os.path.splitext(os.path.basename(args.export_path))[0]
        run_info_filename = f"{export_filename_without_ext}_run_info.txt"
        run_info_path = os.path.join(export_dir, run_info_filename)
        with open(run_info_path, 'w', encoding='utf-8') as f:
            f.write(f"Run time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Command: {' '.join(sys.argv)}\n\n")
            f.write("Args:\n")
