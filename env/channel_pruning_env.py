...
class RankImportance:
    def __init__(self, rank_exps, prunable_groups):
        self.rank_exps = rank_exps  # {layer_name: [rank per channel]}
        # Build group to layer_name mapping
        self.group_to_layer = []
        for group in prunable_groups:
            dep, _ = group[0]
            self.group_to_layer.append(dep.name)

    def __call__(self, group):
        dep, idxs = group[0]
        layer_name = dep.name
        ranks = np.array(self.rank_exps[layer_name])
        return torch.tensor(ranks[idxs], dtype=torch.float32)
...
class ChannelPruningEnv:
    ...
    def __init__(self, model, checkpoint, data, preserve_ratio, args, n_data_worker=4,
                 batch_size=256, export_model=False, use_new_input=False, split_seed=None,
                 rank_exps=None):
        # default setting
        ...
        self.model = copy.deepcopy(self.original_model).cuda()
        ...
        # get prunable groups for state embedding (before pruner)
        dummy_pruner = tp.pruner.BasePruner(
            self.model,
            self.example_inputs,
        ...
        prunable_groups = list(dummy_pruner.DG.get_all_groups(
            ignored_layers=self.ignored_layers,
            root_module_types=[torch.nn.Conv2d, torch.nn.Linear]
        ))
        if rank_exps is not None:
            importance = RankImportance(rank_exps, prunable_groups)
        else:
            importance = tp.importance.GroupMagnitudeImportance(p=1)
    ...
    def _init_data(self):
        ...
        self.train_loader, self.val_loader, n_class = get_split_dataset(self.data_type, self.batch_size,
                                                                        ...
                                                                        shuffle=True, split_seed=self.split_seed)  # same sampling
